#include "../headers/parzpoxarinum.hpp"
#include <cctype>
#include <stdexcept>

std::string 
ParzPoxarinum::encrypt(const std::string& text, const std::string& key) 
{
...
std::string 
ParzPoxarinum::decrypt(const std::string& text, const std::string& key) 
{
    if (text.length() != key.length()) {
        throw std::invalid_argument("Error: The key must be the same length as the message for One-Time Pad cipher.");
    }
